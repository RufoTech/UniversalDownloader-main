...
export default function DownloaderClient({ params }: { params: Promise<{ platform: string }> }) {
  ...
  const jsonLd = {
    ...
    '@type': 'WebPage',
    name: `${config.name} Downloader - Ethereal Downloader`,
    url: `${baseUrl}/${currentLocale}/downloader/${platformKey}`,
    description: t('platform_description', { platform: config.name }),
    breadcrumb: {
      '@type': 'BreadcrumbList',
      itemListElement: [
        {
        ...
        },
        {
          ...
          item: `${baseUrl}/${currentLocale}/downloader/${platformKey}`,
        },
      ],
    },
  };
      ...
      <main className="pt-32 pb-20 px-6 min-h-screen">
        <section className="max-w-7xl mx-auto text-center flex flex-col items-center">
          ...
          {items && items.length > 0 && (
            <div className="w-full max-w-4xl mt-8 flex flex-col gap-6" aria-live="polite">
              {items.map((item, index) => (
                <article key={item.id || index} className="glass-panel p-6 rounded-lg ghost-border shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
